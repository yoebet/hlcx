...
c_query()
{
  ...
  struct in_data da;
  char yy;
  static short in[1]={69};
  static short in0[3]={71,8};
...
while(1)
{
  ...
  {
    note("按卡号查询帐户!");
    sprintf(sql,"SELECT * FROM s_mast WHERE open_br=%d AND card_acc='%s'",da.open_br,da.card_acc);
  }
  ...
  {
    note("按姓名查询帐户!");
    sprintf(sql,"SELECT * FROM s_mast WHERE open_br=%d AND acc_type='K' AND customer like '%%%s%%'",da.open_br,da.customer);
  }
  ...
  for(;;)
  {
    ...
    else if (yy=='d' || yy=='D')
    {
      ...
      if(conn2!=NULL){
        disp_s_det_card(ss->card_acc,ss->open_br,conn2);
      }
    }
  }
...
/***************************
函数名:disp_s_det_card(acc,br)
功  能:显示卡明细
参  数:
       short br  所号
       long card_acc  disp_s_det_card号
返回值:
***************************/
disp_s_det_card(card_acc,br,conn)
char *card_acc;
short br;
MYSQL *conn;
{
  ...
  sprintf(sql,"SELECT * FROM s_detail WHERE card_acc='%s' AND open_br=%d order by det_num",card_acc,br);
  ...
  for(;;)
  {
    mvprintw(ll+ 2,32,"卡    号=[%s]",sd->card_acc);
