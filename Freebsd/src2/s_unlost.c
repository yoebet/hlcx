...
while(1)
{
  ...
  if(mysql_query(conn,"START TRANSACTION")){
	...
	sprintf(sql,"update s_mast set acc_flag='A' WHERE account=%ld and open_br=%d",da.account,da.open_br);
  ...
  refresh();
  mvprintw(22,20,"请插入解挂凭条后按任一键,q退出!");
  refresh();
  qq=getch();
  if (qq!='Q' && qq!='q')
    prt_h_g(&tt,2,1);
  mvprintw(22,20,"请再插入解挂凭条后按任一键,q退出!");
  refresh();
  qq=getch();
  if (qq!='Q' && qq!='q')
    prt_h_g(&tt,2,2);
